...
def main(): 
	...
	os.chdir(subdir);
	os.system("cp ../../g4env4lpc.sh ../bin/simpleDigitizer ../lib/libPFCalEEuserlib.so .")
	os.system("tar -cvzf inputs.tar.gz g4env4lpc.sh simpleDigitizer libPFCalEEuserlib.so")

	listOfFiles = os.listdir("/eos/uscms%s/" % opt.idir);

	ctr = 0;
	for d in listOfFiles:
		listOfSubfiles = os.listdir("/eos/uscms%s/%s" % (opt.idir,d) );
		
		for fo in listOfSubfiles: 
			if "second.root" in fo: 
				subpath = "%s/%s" % (d,fo);
				...
				f1.write("./simpleDigitizer %s test \n" % (fo))
				f1.write("rm %s \n" % (fo))
				f1.write("mv simpleHCalDigis_test.root simpleHCalDigis_%s.root \n" % (ctr))
				f1.write("xrdcp -f simpleHCalDigis_%s.root root://cmseos.fnal.gov/%s/simpleHCalDigis_%s.root \n" % ((ctr),opt.odir,(ctr)) )
				f1.write("rm simpleHCalDigis_%s.root \n" % (ctr));
				f1.close();
