...
void HGCSSCluster::addRecHitFraction(std::pair<HGCSSRecoHit*, double> aHit) {
	std::pair<std::map<HGCSSRecoHit*, double>::iterator, bool> isInserted =
			recHitMap_.insert(aHit);
	if (!isInserted.second)
		isInserted.first->second += aHit.second;
}
...
void HGCSSCluster::Print(std::ostream & aOs) const {
	aOs << std::endl << "=== Layer " << layer_ << "\t width " << width_
			<< "\t Nhits " << recHitMap_.size() << "\t E " << energy_
			<< "\t seedE " << seedE_ << "\t ===" << std::endl << "=== eta,phi "
			<< dir_.eta() << " " << dir_.phi() << "\t seed etadet,phidet "
			<< getSeedEta() << " " << getSeedPhi() << "\t ===" << std::endl;
