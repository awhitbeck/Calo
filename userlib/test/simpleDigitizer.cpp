...
int main(int argc, char** argv) {
	...
	float hpar_mass[100], hpar_px[100], hpar_py[100], hpar_pz[100];
	float hpar_x[100], hpar_y[100], hpar_z[100], hpar_id[100];
	...
	for (unsigned ievt(0); ievt < nEvts; ++ievt) { //loop on entries
		...
		for (unsigned ihit(0); ihit < hitVec->size(); ++ihit){
			...
			// ECAL
			if (hitVec->at(ihit).energy() > 0 && hitVec->at(ihit).layer() < 35 && hitVec->at(ihit).layer() > 0){ 
			...
			// HCAL
			if (hitVec->at(ihit).energy() > 0 && hitVec->at(ihit).layer() > 34){ 
